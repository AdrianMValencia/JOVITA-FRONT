...
import { ModalCompraAjustesComponent } from './modalCompraAjustes/modalCompraAjustes.component';
...
const MODALS: { [name: string]: Type<any> } = {
  compraAjustes: ModalCompraAjustesComponent,
};

@Component({
  selector: 'app-compraAjustes',
  templateUrl: './comprasajustes.component.html',
  providers: [ CompraAjustesService ],
})
export class ComprasAjustesComponent implements OnInit {

  // FormGroup
  fgMain: FormGroup | any;

  compraAjustes: CompraAjustes = new CompraAjustes(0, '', '', '0', '', '', '', '', '', '', true, 1);
  ...
  MainDS: MatTableDataSource<CompraAjustes> = new MatTableDataSource<CompraAjustes>();
  ...
  openModal(name: string) {
    ...
    switch (name) {
      case 'compraAjustes':
        ...
        obj['compraAjustes'] = this.compraAjustes;
  ...
  crudRegistros(){
    this.opcion = 1;
    this.openModal('compraAjustes');
  }

  viewDetail(element: any) {
    this.opcion = 2;
    this.compraAjustes = element;
    this.openModal('compraAjustes');
  }
  ...
  loadMain() {
    ...
    this.productoAjustesService.obtenerProductoAjustes(this.puntoVentas.id).subscribe(response => {
      ...
      this.MainDS = new MatTableDataSource<CompraAjustes>(response.comprasAjustes);
      this.MainDS.paginator = this.pagMain;

      this.MainDS.filterPredicate = function(data: CompraAjustes, filter: string): boolean {
        return data.nombre.trim().toLowerCase().includes(filter);
      };

      this.MainDS.filterPredicate = ((data: CompraAjustes, filter: any ) => {
  ...
  eliminarRegistro(element: CompraAjustes){
