...
import { ModalOrdenRequerimientoProductosComponent } from '../modal-orden-requerimiento-productos/modal-orden-requerimiento-productos.component';
...
@Component({
  selector: 'app-modal-orden-requerimiento',
  templateUrl: './modal-orden-requerimiento.component.html',
  providers: [ OrdenRequerimientoService ]
})
export class ModalOrdenRequerimientoComponent implements OnInit {
...
@Input() fromParent: any;
  ...
  displayedColumns: string[] = [ 'codigoBarra', 'nombre', 'categoria'];
          ...
          if (result.value === 'loadAgain') {
            ...
            if(parseInt(result.opcion) === 1){
              this.detalles.push({
                ...
                idCategoria: result.items.idCategoria,
                categoria: result.items.categoria,
            ...
            if(parseInt(result.opcion) === 2){
              this.detalles.forEach((element, index) => {
                if(index === this.indexEliminar){
                  ...
                  element.idCategoria = result.items.idCategoria,
                  element.categoria = result.items.categoria,
      ...
      if(productosLista.length > 0){
          ...
          if(detalles.length === 0){
            this.detalles.push({
              ...
              categoria: productos.nombreCategoria,
