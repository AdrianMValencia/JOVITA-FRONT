...
import { ModalOrdenPedidoComponent } from './modal-orden-pedido/modal-orden-pedido.component';

// Modals
const MODALS: { [name: string]: Type<any> } = {
  ordenRequerimiento: ModalOrdenPedidoComponent,
...
// FormGroup
  ...
  // PRINCIPAL
  MainDC: string[] = ['pedido', 'fechaPedido', 'puntoVenta', 'vendedor', 'total', 'estadoActual', 'acciones'];
  ...
  new_fgMain(){
    this.fgMain = this.fb.group({
      ...
      usuarios: '',
      productos: '',
      estadoActual: new FormControl([1, 2])
    });
  ...
  ngOnInit() {
    ...
    this.fgMain.get('estadoActual').setValue([1, 2]);
  ...
  downloadPDF(){
    ...
    doc.getLineHeight();
    doc.text("REPORTE ORDEN DE PEDIDO", 140, 30, {align: "center"});
    doc.autoTable({
    ...
    doc.save('Reporte_Orden_Pedido.pdf');
  ...
  downloadExcel(){
    ...
    const title = 'REPORTE ORDEN DE PEDIDO';
    ...
    let worksheet = workbook.addWorksheet('OrdenPedido');
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      fs.saveAs(blob, 'Reporte_Orden_Pedido.xlsx');
    });
