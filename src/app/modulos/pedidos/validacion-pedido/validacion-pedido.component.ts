...
import { ModalValidacionPedidoComponent } from './modal-validacion-pedido/modal-validacion-pedido.component';
// Modals
const MODALS: { [name: string]: Type<any> } = {
  ordenRequerimiento: ModalValidacionPedidoComponent,
  downloadPDF: ModalOrdenRequerimientoPdfComponent
};

@Component({
  selector: 'app-validacion-pedido',
  templateUrl: './validacion-pedido.component.html',
  providers: [ OrdenRequerimientoService ]
})
export class ValidacionPedidoComponent implements OnInit {
 ...
 // FormGroup
  ...
  ngOnInit() {
    ...
    this.fgMain.get('estadoActual').setValue([2,3]);
  ...
  downloadPDF(){
    ...
    doc.getLineHeight();
    doc.text("REPORTE VALIDACIÓN DE PEDIDO", 140, 30, {align: "center"});
    doc.autoTable({
    ...
    doc.save('Reporte_Validacion_Pedido.pdf');
  ...
  downloadExcel(){
    ...
    const title = 'REPORTE VALIDACIÓN DE PEDIDO';
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      fs.saveAs(blob, 'Reporte_Validacion_Pedido.xlsx');
    });
