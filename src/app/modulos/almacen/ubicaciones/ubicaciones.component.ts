...
import { ModalubicacionesComponent } from './modalubicaciones/modalubicaciones.component';
...
const MODALS: { [name: string]: Type<any> } = {
  ubicaciones: ModalubicacionesComponent,
};
...
export class UbicacionesComponent implements OnInit {
   ...
   // PRINCIPAL
   MainDC: string[] = ['idProducto', 'nombre', 'idAlmacen', 'status', 'acciones'];
   MainDS: MatTableDataSource<Ubicaciones> = new MatTableDataSource<Ubicaciones>();
   ...
   openModal(name: string) {
     ...
     switch (name) {
       case 'ubicaciones':
         ...
         obj['ubicaciones'] = this.ubicaciones;
   ...
   crudRegistros(){
     this.opcion = 1;
     this.openModal('ubicaciones');
   }

   viewDetail(element: any) {
     this.opcion = 2;
     this.ubicaciones = element;
     this.openModal('ubicaciones');
   }

   loadMain() {

     this.ubicacionesService.obtenerUbicaciones(this.puntoVentas.id).subscribe(response => {

       this.lista = response.ubicaciones;
       this.MainDS = new MatTableDataSource<Ubicaciones>(response.ubicaciones);
       this.MainDS.paginator = this.pagMain;

       this.MainDS.filterPredicate = function(data: Ubicaciones, filter: string): boolean {
         return data.nombre.trim().toLowerCase().includes(filter);
       };

       this.MainDS.filterPredicate = ((data: Ubicaciones, filter: any ) => {
   ...
   eliminarRegistro(element: Ubicaciones){
     this.funcionesService.mensajeConfirmar('¿Desea eliminar este registro?', '', (result: any) => {
       if (result.isConfirmed) {
         this.ubicacionesService.deleteUbicaciones(element).subscribe(response => {
           this.funcionesService.showLoading();
   ...
   generateData() {
     ...
     data.forEach((element: any) => {
       ...
       result.push([
         ...
         element.idAlmacen == null ? '': element.almacenes.nombre,
         element.nombreAlmacen == null ? '': element.nombreAlmacen,
         element.ubicacion1 == null ? '': element.ubicacion1,
         element.anaquel1 == null ? '': element.anaquel1,
         element.gaveta1 == null ? '': element.gaveta1,
         element.numeroGaveta1 == null ? '': element.numeroGaveta1,
         element.ubicacion2 == null ? '': element.ubicacion2,
         element.anaquel2 == null ? '': element.anaquel2,
         element.gaveta2 == null ? '': element.gaveta2,
         element.numeroGaveta2 == null ? '': element.numeroGaveta2,
   ...
   downloadPDF(){
     ...
     doc.getLineHeight();
     doc.text("REPORTE UBICACIONES", 140, 30, {align: "center"});
     doc.autoTable({
       styles: { lineWidth: 0.2, lineColor: [41, 128, 186]},
       margin: {top: 40},
       head: [[ "PUNTO DE VENTA", "PRODUCTO", "NOMBRE", "ALMACEN", "UBICACION 1", "ANAQUEL 1", "GAVETA 1", "NUMERO GAVETA 1", "UBICACION 2", "ANAQUEL 2", "GAVETA 2", "NUMERO GAVETA 2", "OBSERVACIONES", "ESTADO"]],
       body: this.generateData()
     });
     doc.save('Reporte Ubicaciones.pdf');
   ...
   downloadExcel(){
     ...
     const title = 'REPORTE UBICACIONES';
     const header = [ "PUNTO DE VENTA", "PRODUCTO", "NOMBRE", "ALMACEN", "UBICACION 1", "ANAQUEL 1", "GAVETA 1", "NUMERO GAVETA 1", "UBICACION 2", "ANAQUEL 2", "GAVETA 2", "NUMERO GAVETA 2", "OBSERVACIONES", "ESTADO"];
     const data = this.MainDS.filteredData;
     let lista: any[] = [];

     let workbook = new Workbook();
     let worksheet = workbook.addWorksheet('Ubicaciones');
     ...
     workbook.xlsx.writeBuffer().then((data: any) => {
       ...
       fs.saveAs(blob, 'Reporte Ubicaciones.xlsx');
     });
