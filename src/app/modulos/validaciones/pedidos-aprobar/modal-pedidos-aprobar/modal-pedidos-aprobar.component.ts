...
import { ModalOrdenRequerimientoProductosComponent } from 'src/app/modulos/pedidos/orden-requerimiento/modal-orden-requerimiento-productos/modal-orden-requerimiento-productos.component';
import { OrdenRequerimiento } from 'src/app/modulos/pedidos/orden-requerimiento/model/ordenRequerimiento';
import { OrdenRequerimientoDetalles } from 'src/app/modulos/pedidos/orden-requerimiento/model/ordenRequerimientoDetalles';
import { OrdenRequerimientoService } from 'src/app/modulos/pedidos/orden-requerimiento/service/orden-requerimiento.service';
...
@Component({
  selector: 'app-modal-pedidos-aprobar',
  templateUrl: './modal-pedidos-aprobar.component.html',
  providers: [ OrdenRequerimientoService  ]
})
export class ModalPedidosAprobarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (opc === 2) {
      ...
      this.titulo = 'Modificar Pedidos por Aprobar ' + array.id.toString().padStart(4, '0');
      ...
      if(array.detalles !== null){
       ...
       this.titulo = 'Agregar Pedidos por Aprobar';
    }
  }
  ...
  saveOrdenRequerimiento(form: FormGroup): any {
    ...
    if (form.invalid) {
      ...
      if (this.fromParent.opcion === '1' || this.fromParent.opcion === 1) {
        titulo = '¿Estas seguro de generar el Pedidos por Aprobar?';
      }else{
        titulo = '¿Estas seguro de modificar el Pedidos por Aprobar?';
      }
