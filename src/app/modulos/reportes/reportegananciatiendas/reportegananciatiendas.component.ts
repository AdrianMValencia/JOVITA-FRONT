...
@Component({
  selector: 'app-reportegananciatiendas',
  templateUrl: './reportegananciatiendas.component.html',
  providers: [ReportesService]
})
export class ReportegananciatiendasComponent implements OnInit {
  // FormGroup
  ...
  // PRINCIPAL
  MainDC: string[] = ['puntoventa', 'fecha', 'venta', 'compra', 'ganancia'];
  ...
  loadMain() {
    ...
    this.service.cargarReporteGananciaTiendas(this.formGroup.get('fechaIni').value, this.formGroup.get('fechaFin').value, this.puntoVentas.id).subscribe(response => {
  ...
  generateData() {
    ...
    data.forEach((element: any) => {
      ...
      result.push([
        ...
        element.venta == null ? '': parseFloat(element.venta).toFixed(2),
        element.compra == null ? '': parseFloat(element.compra).toFixed(2),
        element.ganancia == null ? '': parseFloat(element.ganancia).toFixed(2)
      ]);
  ...
  downloadPDF(){
    ...
    doc.getLineHeight();
    doc.text("REPORTE VENTAS TOTALES POR MES", 140, 30, {align: "center"});
    doc.autoTable({
      styles: { lineWidth: 0.2, lineColor: [41, 128, 186]},
      columnStyles: { 2: { halign: 'right' }, 3: { halign: 'right' }, 4: { halign: 'right' } },
      margin: {top: 40},
      head: [["PUNTO VENTA", "FECHA", "VENTA", "COMPRA", "GANANCIA"]],
      body: this.generateData()
    });
    doc.save('Reporte Ventas Totales por Mes.pdf');
  ...
  downloadExcel(){
    ...
    const title = 'REPORTE VENTAS TOTALES POR MES';
    const header = ["PUNTO VENTA", "FECHA", "VENTA", "COMPRA", "GANANCIA"];
